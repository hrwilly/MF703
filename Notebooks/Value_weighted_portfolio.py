...
# filter top 1%
filtered_market_caps, filtered_returns = filter_top_stocks_by_market_cap(size, return_df, 0.99)
ABNR_filtered_quantile_returns = get_quantile_returns(ab_nr_df, filtered_returns, filtered_market_caps)
ABNR_filtered_excess = pd.DataFrame({'Excess': safe_round(ABNR_filtered_quantile_returns.mean(), 2)})
ABNR_filtered_CAPM = cal_CAPM(ABNR_filtered_quantile_returns, ff3_df)
ABNR_filtered_FF3 = cal_Fama_French(ABNR_filtered_quantile_returns, ff3_df)
ABNR_filtered_result = result_report(ABNR_filtered_excess, ABNR_filtered_CAPM, ABNR_filtered_FF3).T
ABNR_filtered_result.to_csv('sp500_filtered_ABNR_result.csv')

ABPR_filtered_quantile_returns = get_quantile_returns(ab_pr_df, filtered_returns, filtered_market_caps)
ABPR_filtered_excess = pd.DataFrame({'Excess': safe_round(ABPR_filtered_quantile_returns.mean(), 2)})
ABPR_filtered_CAPM = cal_CAPM(ABPR_filtered_quantile_returns, ff3_df)
ABPR_filtered_FF3 = cal_Fama_French(ABPR_filtered_quantile_returns, ff3_df)
ABPR_filtered_result = result_report(ABPR_filtered_excess, ABPR_filtered_CAPM, ABPR_filtered_FF3).T
ABPR_filtered_result.to_csv('sp500_filtered_ABPR_result.csv')
